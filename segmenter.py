...
def open_image(image_file):
    ...
    image_np = cv2.resize(image_np, (0, 0), fx=0.25, fy=0.25, interpolation=cv2.INTER_AREA)
    return image_np
...
def process_images(masked_image, original_image, model):
   ...
   # add median blur
    #original_image = cv2.medianBlur(original_image, 1)

    # ADD BİLATERAL FILTER
    ...
    u_diff = distance(u_component, non_masked_area_luv[1])
    v_diff = distance(v_component, non_masked_area_luv[2])
    ...
    # add kmeans clustering to y_pred
        ...
        if masked_image_file and original_image_file and model_file:
            try:
                with st.spinner('Processing images...'):
                    ...
                    np.savetxt(f'result-wse2.txt', result, fmt='%d')

                fig, ax = plt.subplots(figsize=(12, 8))
                im = ax.imshow(result, cmap='turbo', interpolation='none')
